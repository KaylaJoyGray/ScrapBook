...
std::vector<point> get_area(point &p1, point &p2, dyn_map &map)
...
// p1 is top left point, p2 is bottom right point
{
	...
	for (int y = Start; y >= End; y--) {
		...
		next_line = get_line(next_p2, next_p1, map);
...
void draw_line(int value, point &start, point &end, dyn_map &map)
...
void draw_area(int value, point &start, point &end, dyn_map &map)
// pushes a filled rectangle to a tile map with value
{
	std::vector<point> point_list;
	int dist = distance(start,end)+1;
	
	point_list.reserve(dist*dist);
	
	point_list = get_area(start, end, map);
